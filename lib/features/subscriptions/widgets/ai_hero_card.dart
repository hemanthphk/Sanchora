...
class AiHeroCard extends StatelessWidget {
  ...
  const AiHeroCard({super.key, required this.subscriptions});

  @override
  Widget build(BuildContext context) {
    final activeCount =
        SubscriptionService.instance.dashboardSummary.activeSubscriptions;
    ...
    for (var sub in subscriptions) {
      ...
      categorySpending[sub.category] =
          (categorySpending[sub.category] ?? 0) + sub.monthlyPrice;

      if (sub.monthlyPrice >= 300 && sub.billingCycle == BillingCycle.monthly) {
        potentialSavings +=
            (sub.monthlyPrice *
            0.2); // Rough estimation of savings if switched to annual
  ...
  Widget _buildInsightContent(
    ...
    for (var sub in subscriptions) {
      if (sub.status == SubscriptionStatus.active ||
        ...
        if (sub.isTrial ||
            sub.name.toLowerCase().contains('trial') ||
            sub.currentPrice == 0) {
          continue;
        }
        validActiveCount++;
    ...
    final avgCost = validActiveCount > 0
        ? totalMonthlyEquivalent / validActiveCount
        : 0.0;
    ...
    insights.add(
      Text(
        'Average monthly cost is $avgCostStr per subs.',
        style: TextStyle(
          fontSize: 14,
          fontWeight: FontWeight.w500,
          color: Colors.white.withValues(alpha: 0.9),
